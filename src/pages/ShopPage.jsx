...
import { FiArrowRight, FiChevronDown, FiFilter, FiGrid, FiSearch, FiSliders, FiTag, FiX } from 'react-icons/fi'
...
const filterShell = 'rounded-2xl border border-slate-100 bg-white/90 shadow-sm'
...
export default function ShopPage() {
  ...
  const clearFilters = () => {
    setFilters({ category: '', brand: '', minPrice: '', maxPrice: '', condition: '', search: '', sort: 'createdAt', order: 'DESC', isFeatured: '', isFlashSale: '' })
  ...
  const activeFilters = [
    ...
    filters.isFlashSale && 'Flash sale',
  ].filter(Boolean)
  ...
  const Choice = ({ active, children, onClick }) => (
    <button
      type="button"
      onClick={onClick}
      className={`w-full flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold transition-all ${
        active ? 'bg-blue-50 text-primary ring-1 ring-blue-100' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
      }`}
    >
  ...
  const FilterPanel = () => (
    <div className="space-y-6">
      <div>
        <label className="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Search</label>
      ...
      <div>
        ...
        <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
          <Choice active={!filters.category} onClick={() => setFilter('category', '')}>All Categories</Choice>
          {categories?.map(cat => (
            <Choice key={cat.id} active={String(filters.category) === String(cat.id)} onClick={() => setFilter('category', cat.id)}>{cat.name}</Choice>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1 max-h-56 overflow-y-auto pr-1">
          <Choice active={!filters.brand} onClick={() => setFilter('brand', '')}>All Brands</Choice>
          {brands?.map(b => (
            <Choice key={b.id} active={String(filters.brand) === String(b.id)} onClick={() => setFilter('brand', b.id)}>{b.name}</Choice>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input
            ...
            placeholder="Min"
            type="number"
            className="w-full rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-primary focus:bg-white"
          />
          <input
          ...
          {priceRanges.map(([min, max, label]) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl px-3 py-2 text-xs font-bold transition-all ${active ? 'bg-primary text-white shadow-md shadow-primary/20' : 'bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-primary'}`}
              >
                {label}
              </button>
            )
        ...
        <div className="grid grid-cols-2 gap-2">
          {['', 'new', 'refurbished', 'used'].map(c => (
            <button
              ...
              className={`rounded-xl px-3 py-2.5 text-sm font-bold capitalize transition-all ${filters.condition === c ? 'bg-primary text-white shadow-md shadow-primary/20' : 'bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-primary'}`}
            >
              {c || 'All'}
            </button>
          ))}
      ...
      {activeFiltersCount > 0 && (
        <button onClick={clearFilters} className="w-full py-3 rounded-2xl border border-red-100 bg-red-50 text-red-500 text-sm font-bold hover:bg-red-100 transition-colors">
          Clear all filters
        </button>
      )}
    </div>
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/40">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
            <div className="max-w-3xl">
              <motion.span initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs md:text-sm font-bold text-cyan-100 border border-white/15 backdrop-blur-md">
                <FiGrid className="w-3.5 h-3.5 md:w-4 md:h-4" /> Ozone Lapcare store
              </motion.span>
              <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="mt-3 md:mt-5 text-2xl md:text-4xl font-black text-white tracking-tight">
                Find the right laptop gear faster.
              </motion.h1>
              <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }} className="mt-2 md:mt-4 text-slate-300 leading-relaxed max-w-2xl text-xs md:text-base">
                Browse laptops, parts, upgrades and accessories with focused filters, trusted brands and clear product details.
              </motion.p>
            </div>
          </div>
        </section>
        ...
        <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 md:py-8">
          <div className={`${filterShell} p-3 md:p-5 mb-4 md:mb-6`}>
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 md:gap-4">
              <div>
                ...
                <h2 className="mt-0.5 md:mt-1 text-lg md:text-xl font-black text-slate-900">All Products</h2>
              </div>

              <div className="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-3">
                <button onClick={() => setShowFilters(true)} className="lg:hidden inline-flex items-center justify-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-700 relative">
                  <FiSliders className="w-3.5 h-3.5 md:w-4 md:h-4" /> Filters
                  {activeFiltersCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-5 h-5 gradient-bg text-white text-[10px] rounded-full flex items-center justify-center font-bold">{activeFiltersCount}</span>}
                </button>
          ...
          <div className="flex gap-8">
            <aside className="hidden lg:block w-72 shrink-0">
              <div className={`${filterShell} p-6 sticky top-24`}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  {activeFiltersCount > 0 && <button onClick={clearFilters} className="text-xs text-red-500 hover:text-red-600 font-bold">Clear</button>}
                </div>
                <FilterPanel />
              </div>
            </aside>
