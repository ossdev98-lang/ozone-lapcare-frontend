...
export default function ProductCard({ product, compact }) {
  ...
  if (compact) {
    return (
      ...
      >
        <div
          className="product-card block group w-full"
          onClick={isUnavailable ? undefined : handleCardClick}
          onKeyDown={isUnavailable ? undefined : handleCardKeyDown}
          role={isUnavailable ? 'article' : 'link'}
        ...
        >
          <div className="product-img-wrap relative">
            ...
            {isUnavailable && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <span className="bg-white/90 text-[#111827] text-[10px] sm:text-xs font-bold px-2 py-1 rounded-lg backdrop-blur-sm">Currently Unavailable</span>
              </div>
            )}
          ...
          <div className="p-2.5">
            ...
            <div className="min-w-0">
              {!isUnavailable && <span className="text-sm font-bold text-[#111827] truncate block">{formatPrice(product.isFlashSale && product.flashSalePrice ? product.flashSalePrice : product.price)}</span>}
              ...
              <motion.button
                whileHover={{ scale: isUnavailable ? 1 : 1.05 }} whileTap={{ scale: isUnavailable ? 1 : 0.95 }}
                onClick={isUnavailable ? undefined : handleCart}
                disabled={cartLoading || product.stock === 0 || isUnavailable}
              ...
              >
                {cartLoading ? <span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <>{isUnavailable ? 'Unavailable' : product.stock === 0 ? 'Out' : 'Add'}</>}
              </motion.button>
            </div>
          </div>
        </div>
    ...
    >
      ...
      >
        <div className="product-img-wrap relative">
          ...
          {isUnavailable && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
              <span className="bg-white/90 text-[#111827] text-xs sm:text-sm font-bold px-3 py-1.5 rounded-lg backdrop-blur-sm">Currently Unavailable</span>
            </div>
          )}
          {discount > 0 && !isUnavailable && (
          ...
          {product.isFlashSale && !isUnavailable && (
          ...
          {product.condition === 'refurbished' && !isUnavailable && (
        ...
        <div className="p-3 sm:p-4">
          ...
          {product.avgRating > 0 && !isUnavailable && (
          ...
          <div className="flex items-center justify-between mt-2 sm:mt-3 gap-2">
            <div className="min-w-0">
              {!isUnavailable && <span className="text-base sm:text-lg font-bold text-[#111827] truncate block">{formatPrice(product.isFlashSale && product.flashSalePrice ? product.flashSalePrice : product.price)}</span>}
              {product.comparePrice && !isUnavailable && (
            ...
            >
              ...
              {isUnavailable ? 'Unavailable' : product.stock === 0 ? 'Out' : 'Add'}
            </motion.button>
          </div>

          {product.stock > 0 && product.stock <= 5 && !isUnavailable && (
            <p className="text-[10px] sm:text-xs text-amber-500 mt-1 sm:mt-1.5 font-medium">Only {product.stock} left!</p>
          )}
          {product.stock === 0 && !isUnavailable && (
            <p className="text-[10px] sm:text-xs text-red-500 mt-1 sm:mt-1.5 font-medium">Out of stock</p>
          )}
          {isUnavailable && (
            <p className="text-[10px] sm:text-xs text-slate-500 mt-1 sm:mt-1.5 font-medium">Currently unavailable</p>
          )}
        </div>
      </div>
    </motion.div>
