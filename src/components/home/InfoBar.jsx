        ...
        <div className="grid grid-cols-1 gap-2.5 sm:gap-4">
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-6">
            {trustItems.map(([Icon, text], i) => (
              <div key={text}
                className={`group flex items-center gap-1.5 text-white ${i === 0 ? 'justify-start' : i === 1 ? 'justify-center' : 'justify-end'}`}>
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white/15 ring-1 ring-white/20">
                  <Icon className="w-2.5 h-2.5" />
                </span>
                <span className="text-[9px] sm:text-xs font-semibold tracking-wide leading-tight">{text}</span>
              </div>
            ))}
          </div>
          ...
          {/* Stats row: single line on mobile, 4-pos on desktop */}
          <div className="flex items-center justify-start gap-5 overflow-x-auto scrollbar-hide sm:grid sm:grid-cols-4 sm:gap-x-0 sm:overflow-visible sm:justify-center">
